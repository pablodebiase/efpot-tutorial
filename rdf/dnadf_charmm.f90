...
character crdfile*256,dcdfile*256,mskfile*256,line*256,dcdfmt*4
integer ul(256),ll(256),num
...
do i=1,len(bs)
  bs(i:i)=achar(8)
enddo
write(*,'(/A)')'DNADF version 2.50 computes the Cylindrical Distribution Function (CDF) between the DNA fragments and free particles.'
write(*,'(A)') '      It also computes the Radial Distribution Function (RDF) between the DNA fragments and free particles.'
write(*,'(A)') '      Created by Pablo M. De Biase (pablodebiase@gmail.com) since Abril 2011'
write(*,'(/A/)') 'NOTE: Press ENTER to accept the default option in between []'
write(*,'(A$)') 'Input CHARMM coordinates (.crd) filename: '
read(*,*) crdfile

! Read crd
call readcharmmcrd(crdfile)

write(*,'(A$)') 'Input CHARMM trajectory dcd or dcde filename (filename format): '
read(*,*) dcdfile,dcdfmt

if (dcdfmt.eq.'dcde'.or.dcdfmt.eq.'DCDE') then
  ext=.true.
else
  ext=.false.
endif

write(*,'(A$)') 'Compute CDF (y/n) [n]? '
read(*,'(A)') line
...
write(*,'(A$)') 'Compute RDF for DNA-IONS (y/n) [y]? '
read(*,'(A)') line
...
write(*,'(A$)') 'Compute RDF for IONS (y/n) [y]? '
read(*,'(A)') line
if (line(1:1).eq.'n'.or.line(1:1).eq.'N') then
  dordfion=.false.
else
...
if (dordf.or.docdf) then
  write(*,'(A$)') 'Input Fragment Mask (.msk) filename [HELP and END]: '
  read(*,'(A)') mskfile
...
write(*,'(A$)') 'Input resolution factor in Angstroms [0.1]: '
read(*,'(A)') line
...
if (dordf.or.docdf) then 
  ...
  write(*,'(A$)') 'Define center of coarse particles at geometric center (y/n) [n] ? '
  read(*,'(A)') line
...
write(*,'(A$)') 'Residue Type Number sequence for ions/solvent: '
read(*,'(A)') line
...
if (dordf.or.docdf) then 
  ...
  write(*,'(A$)') 'Substract DNA volume to the periodic cell volume (y/n)? [n]: '
  read(*,'(A)') line
  if (line(1:1).eq.'y'.or.line(1:1).eq.'Y') then
    cdv=.true.
  else
...
if (docdf) then
  write(*,'(A$)') 'Select the fragment group number to be used as a fixed Reference Axis (RA) [ENTER to use each fragment group as a RA] : '
  read(*,'(A)') line
  if (len_trim(line).eq.0) then
    fra=0
  else 
...
write(*,'(A,F6.1,A$)') 'Choose max distance limit in Ang to compute distribution functions [',bb/2+delr,']: '
read(*,'(A)') line
if (len_trim(line).eq.0) then
...
if (docdf) then
  write(*,'(A$)') 'Consider terminal residues for CDF (y/n) [y]? '
  read(*,'(A)') line
  if (line(1:1).eq.'n'.or.line(1:1).eq.'N') then
    termon=.false.
  else
...
write(*,'(A$)') 'Write PDB with all elements (y/n) [n]? '
read(*,'(A)') line
wout=.false.
